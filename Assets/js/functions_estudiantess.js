let tableEstudiantess;
//let rowTable = ""; //paremetro del actualizar
let divLoading = document.querySelector("#divLoading");
document.addEventListener('DOMContentLoaded', function(){
	
    //data tables 
	tableEstudiantess = $('#tableEstudiantess').dataTable( {
        "aProcessing":true,
        "aServerSide":true,
        "language": {
        ...
        "ajax":{
            "url": " "+base_url+"/Estudiantess/getEstudiantes",/*:P*/
            "dataSrc":""
		},
        "columns":[
            //{"data":"idestudiante"},
				//{"data":"identificacion"},
					//{"data":"nombres"},
						//{"data":"apellidos"},
							...
							if(document.querySelector("#formEstudiantess")){
								let formEstudiantess = document.querySelector("#formEstudiantess");
								formEstudiantess.onsubmit = function(e) {
									e.preventDefault();
									...
									let ajaxUrl = base_url+'/Estudiantess/setEstudiante'; /*:P*/
									let formData = new FormData(formEstudiantess);
									...
									request.onreadystatechange = function(){
										if(request.readyState == 4 && request.status == 200){
											...
											if(objData.status)
											{
												...
												$('#modalFormEstudiantess').modal("hide");
												formEstudiantess.reset();
												swal("Usuarios", objData.msg ,"success");
												tableEstudiantess.api().ajax.reload();//recarga la tabla
												}else{
					...
					function fntViewInfo(idpersona){  /*:p*/
						...
						let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
						let ajaxUrl = base_url+'/Estudiantess/getEstudiante/'+idpersona; /*:p*/
					...
					function fntEditInfo(/*element,*/ idpersona) //variable del parametro del inicio 
					{
						...
						request.onreadystatechange = function(){
							...
							$('#modalFormEstudiantess').modal('show');/*:p*/
						}
					}
					...
					function fntDelInfo(idpersona){
						swal({
							...
							{
								let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
								let ajaxUrl = base_url+'/Estudiantess/delEstudiante';/*:P*/
								...
								request.onreadystatechange = function(){
									if(request.readyState == 4 && request.status == 200){
										...
										if(objData.status)
										{
											...
											tableEstudiantess.api().ajax.reload();
											}else{
					...
					function fntGradoEstudiantes(){
						if(document.querySelector('#listGradoid')){
							let ajaxUrl = base_url+'/Estudiantess/getSelectGrado';/*:P*/
							let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
							request.open("GET",ajaxUrl,true);
							request.send();
					...
					function openModal()
					{
						...
						document.querySelector("#formEstudiantess").reset();
						$('#modalFormEstudiantess').modal('show');/*:P*/
					}										